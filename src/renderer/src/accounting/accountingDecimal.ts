 ...
 * present ("0", "0.01", "0.29", "10.29" all valid; "10.", ".5",
 * "10.299", "-5", "+1", "1e5", "abc", "" all invalid). No sign of any
 * kind is accepted -- debit/credit minor units are always non-negative,
 * mirroring journal_entry_lines' own CHECK constraint exactly.
 */
const ACCOUNTING_DECIMAL_INPUT_PATTERN = /^(\d+)(?:\.(\d{1,2}))?$/
 ...
 * anything that doesn't match ACCOUNTING_DECIMAL_INPUT_PATTERN.
 *
 ...
 * integer -- this is exact integer arithmetic on digit characters,
 * never a floating-point multiplication, so there is no rounding step
 * to get wrong in the first place. Mirrors productDecimal.ts's own
 * parseDecimalToMinorUnits exactly; a dedicated copy for the accounting
 * domain rather than a shared import, following this codebase's own
 * established per-domain-copy convention (supplierPriceDecimal.ts,
 * customerCreditDecimal.ts each have their own copy too).
 */
export function parseDecimalToMinorUnits(rawInput: string): number | undefined {
  const trimmed = rawInput.trim()
  const match = ACCOUNTING_DECIMAL_INPUT_PATTERN.exec(trimmed)
  if (!match) {
    return undefined
  }
