...
describe('firstRunSetupService', () => {
  ...
  describe('the happy path', () => {
    ...
    it('produces exactly 15 audit rows: company, 11 numbering rules, owner user, owner role, recovery credential', async () => {
      const service = createFirstRunSetupService()
      const commitToken = await prepareAndConfirm(service)
    ...
    it('all 11 approved numbering-rule rows exist with current_sequence_value = 0 and the correct definitions', async () => {
      const service = createFirstRunSetupService()
      const commitToken = await prepareAndConfirm(service)
      await service.completeSetup(db, VALID_COMPANY, VALID_OWNER, commitToken)
